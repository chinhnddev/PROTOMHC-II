"""Run inference for ESM2 Frozen Transformer on the test split and save predictions."""
import argparse
import os
import joblib
import torch
...
def parse_args():
    parser = argparse.ArgumentParser(description="Predict test split with ESM2 Frozen Transformer")
    parser.add_argument(
        "--ckpt",
        type=str,
        default="checkpoints/ESM2_Frozen_Transformer/best.ckpt",
        help="Path to Lightning checkpoint (.ckpt)",
    )
    parser.add_argument(
        "--out",
        type=str,
        default="results/predictions/esm2_transformer_test_preds.pkl",
...
def main():
    ...
    model = ESM2FrozenTransformer.load_from_checkpoint(args.ckpt, map_location=device)
    model.to(device)
    model.eval()
