"""Run inference for ProtBERT Transformer on the test split and save predictions."""
import argparse
import os
import joblib
import torch
...
def parse_args():
    parser = argparse.ArgumentParser(description="Predict test split with ProtBERT Transformer")
    parser.add_argument(
        "--ckpt",
        type=str,
        default="checkpoints/ProtBERT_Transformer/best.ckpt",
        help="Path to checkpoint (.ckpt)",
    )
    parser.add_argument(
        "--out",
        type=str,
        default="results/predictions/protbert_test_preds.pkl",
...
def main():
    ...
    model = ProtBERTTransformer.load_from_checkpoint(args.ckpt, map_location=device)
    model.to(device)
    model.eval()
